...
| `/ai-goal` | 选定并设计当前目标，到方案确认 | 否 | 准备开始一个具体目标时 |
| `/ai-check` | 当前目标健康检查：状态一致、约束、缺口 | 否 | 实现前/中/后做流程检查时 |
| `/ai-sync` | 把目标结果回写 roadmap + 变更记录，并协助提交 | 否 | 当前目标完成并验证后 |
...
框架要求任意时刻只有一个当前目标。`/ai-goal` 的推进逻辑：
...
- 用户确认后才进入实现
- 实现与 TDD 交给正常对话，不由命令强制驱动
- 测试通过后再用 `/ai-sync` 同步回 `roadmap.md`

目的是让 AI 不会直接跳进代码，而是先和用户把当前目标说清楚。
